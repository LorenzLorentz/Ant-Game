...
### 1.1 游戏规则（简述）
- 棋盘大小 `19 x 19`（见 logic/constant.py），地形包括：平原、沼泽、山地。
...
- 胜负判定（logic/game_rules.py）：
  - 若任一方主将被消灭，另一方立即获胜；
  - 若回合数 ≤ 512，游戏未结束；
  - 超过 512 回合采用平局裁决：优先比较基地血量、击败对方蚂蚁数、使用超级武器次数、最后先手胜。
...
- 最大回合数：`512`；超过后进入平局裁决。
- 蚂蚁寿命上限：年龄 `> 32` 时死亡。
- 基地产蚁节奏：主基地产能等级 `1/2/3` 分别对应每 `4/2/1` 回合产出一只蚂蚁。
- 科技初始值：行动力 `2`，攀岩 `0`，沼泽免疫 `0`，超级武器解锁 `0`。
- 玩家初始行动力：每位玩家每回合 `2` 步军队行动。
- 主将/副将/农夫初始数量：双方主将共 `2` 个，野生副将 `4` 个，农夫 `8` 个。
- 评测时间上限： `1` 秒。
- 评测内存上限：`512 MB`。
